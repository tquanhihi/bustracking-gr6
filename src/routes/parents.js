...
router.get('/list', async (req, res) => {
  try {
    const [rows] = await pool.query("SELECT * FROM parents ORDER BY parent_id DESC");
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const newParent = { parent_id: result.insertId, full_name, phone, email };
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    const updatedParent = { parent_id: parseInt(id), full_name, phone, email };
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    req.app.get('io')?.emit('parentDeleted', { parent_id: parseInt(id) });
